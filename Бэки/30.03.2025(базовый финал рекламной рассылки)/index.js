...
mongoose.connect('mongodb://127.0.0.1:27017/wowbot')
    .then(() => {
        console.log('Мы подключились к БД бота!!!');
    })
    .catch((error) => {
...
bot.onText(/\/start/, async (msg) => {
    ...
    const isAdmin = ADMIN_IDS.includes(msg.chat.id);

    const fullKeyboard = kb_text.home[0]; // вся клавиатура
    const keyboard = isAdmin
        ? fullKeyboard
        : fullKeyboard.filter(row => !row.includes(kb.home.broadcast)); // удалим только строку с этой кнопкой
...
// Подключаем файл с информационной рассылкой
setupBroadcast(bot, User);